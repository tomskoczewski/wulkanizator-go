...
- [ ] **Auth → URL Configuration**: set **Site URL** to `https://wulkanizator-go.<your-subdomain>.workers.dev` (fill in after Phase 3 if not known yet — but come back and set it, otherwise magic-link and email-confirmation redirects go to `localhost`).
...
- [ ] `https://wulkanizator-go.<subdomain>.workers.dev` returns 200 for `/`.
- [ ] Signup → email confirm (if on) → signin → `/dashboard` all succeed against the live URL.
- [ ] `npx wrangler tail` shows structured request logs and zero unhandled exceptions during the smoke walkthrough.
- [ ] `npx wrangler versions list` shows at least two versions (Phase 3 + Phase 5 rollback drill).
- [ ] A push to `main` triggers a Cloudflare-side build that ends with a green "Deployment successful" status and the live URL reflects the change within ~2 minutes.
- [ ] Supabase Dashboard → Authentication → Users shows the test user(s) created via the live flow.
