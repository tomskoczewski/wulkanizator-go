...
## A `security definer` RPC that unconditionally inserts accumulates a row on every successful call
...
## Never key test data on a timestamp alone — add the worker's identity

- **Context**: Any test that creates rows in a shared database and tears them down by a self-generated key — the `e2e/` suite under parallel Playwright workers, and any future integration suite that seeds its own fixtures.
- **Problem**: Two specs starting in the same millisecond in different workers received an identical `Date.now()` stamp. Teardown deletes by that stamp, so the spec that finished first swept away the other spec's appointment mid-assertion — surfacing as an intermittent, unreproducible "the row just isn't on the page" failure that looks like a product bug.
- **Rule**: Never build a test-data key from a timestamp alone. Combine it with the worker's own identity (Playwright: `testInfo.parallelIndex`) so that two tests sharing a millisecond cannot share a key — and delete by that full key, never by a prefix.
- **Applies to**: all
